...
function flatten(data) {
    ...
    let oneDArray = [];
    data.forEach(el => {
        return oneDArray.push(...flatten(el));
    });
    return oneDArray;
}
...
function pathFinder(directories, targetFile) {
    for (let fileOrFolder in directories) {
        if (fileOrFolder === targetFile) return "/" + targetFile;
        let subdirectory = directories[fileOrFolder];
        let subPath = pathFinder(subdirectory, targetFile);
        if (subPath != null) return fileOrFolder + subPath;
    }
    return null;
}
