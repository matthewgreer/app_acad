...
def small_and_wealthy
  ...
  execute(<<-SQL)
    ...
    FROM
      countries
    WHERE
      countries.gdp > 5000000000 AND countries.area < 2000
  SQL
end
...
def scandinavia
  ...
  execute(<<-SQL)
    ...
    FROM
      countries
    WHERE
      countries.name = 'Denmark' OR countries.name = 'Finland' OR countries.name = 'Norway' OR countries.name = 'Sweden'
  SQL
end
...
def just_the_right_size
  # Show the country OR the area in 1000's of square kilometers for countries
