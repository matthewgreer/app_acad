...
Function.prototype.myBind = function (context, ...bindArgs) {
  ...
  const that = this; // saves the context of the original function we are binding
  // fat arrow takes this from the declaration scope
  return function(...callArgs) { // can close over function parameters (context, bindArgs)
    // this becomes whatever the context should be depending on how it is invoked
...
Function.prototype.myCurry = function(numArgs) {
  const args = [];
  const fn = this; // refers to the function we invoke .curry on

  return function _myCurry(arg) { // closes over args, fn, and numArgs variables
    // debugger;
    args.push(arg);

    if (args.length === numArgs) {
      // as the curried function will be invoked function style, 'this' would be the gloabl object/window
      // luckily we saved our original context to the fn variable
      
      return fn.apply(null, args); 
      
      // .apply() and .call() take a context as their first argument, we can just pass 'null'
      
      // return fn.call(null, ...args); // another way to invoke w/ call
      
      // return fn(...args); // can also invoke with spread out arguments normally
