class User < ApplicationRecord
  ...
  def password=(password)
    ...
    self.password_digest = BCrypt::Password.create(password)
      # BCrypt::Password.create creates a password digest (hash)
  end

  def is_password?(password)
    BCrypt::Password.new(self.password_digest).is_password?(password)
      # BCrypt::Password.new instantiates a BCrypt password object and reads password_digest back to original pwrd
      # keep in mind BCrypt::Password.is_password? is different from User#is_password?
  end
